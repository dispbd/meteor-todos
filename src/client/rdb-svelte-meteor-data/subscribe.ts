...
import { current_component } from 'svelte'

const _subscribe = Meteor.subscribe
Meteor.subscribe = function subscribe(name) {
  ...
  if (params.length > 1) {
    ...
    if (last) {
      ...
      if (typeof last === 'function') {
        ...
        [last.onReady, last.onError, last.onStop].some(
          (f) => typeof f === 'function',
        )
      ) {
  ...
  if (current_component) {
    current_component.$$.on_destroy.push(
      subscription.stop.bind(subscription),
    )
  }
